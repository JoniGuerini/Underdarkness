...
export type SocialChannel = 'global' | 'guilda';
...
let _id = 0;
function msg(kind: MessageKind, time: string, author: ChatPlayer | undefined, segments: MessageSegment[]): ChatMessage {
  return { id: `m${++_id}`, kind, time, author, segments };
}
...
export const GLOBAL_MESSAGES: ChatMessage[] = [
  msg('global', '14:02', acoFrio, [t('alguém vd '), it('Erva Vermelha', 'comum'), t(' em pdg? solana ta cobrando 6g, ta de zoa')]),
  msg('global', '14:03', noobMage, [t('como faço pra subir dano de magias')]),
  msg('global', '14:03', erudita, [t('@noobMage123 equipa cajado e itens com '), it('+ % de Dano de Magias', 'magico'), t(' ou '), it('+ % de Dano de Magias de Fogo', 'magico'), t(' no sufixo')]),
  msg('global', '14:04', selvinha, [t('lfg '), qt('A Coleção do Erudito', 'bounty'), t(' 4/5')]),
  msg('system', '14:05', undefined, [t('Servidor Pedragal: '), t('1.247 jogadores online')]),
  msg('global', '14:06', mestreEspada, [t('VENDO '), it('Lâmina do Vau', 'magico'), t(' +8 dano fis +3 agi — fz proposta')]),
  msg('global', '14:07', botaVau, [t('alguém indica build pra ladino solo? to apanhando feio')]),
  msg('global', '14:07', diabloFan, [t('@Bota_do_Vau agi cap depois critico, simples')]),
  msg('global', '14:08', vinhoFraco, [t('puta que pariu o boss do pântano')]),
  msg('global', '14:09', k4t4, [t('drop rate de '), it('Raiz Noturna', 'comum'), t(' ta ridículo hj. fiz a cripta 6 vezes nada')]),
  ...
  msg('global', '14:12', acoFrio, [t('compro '), it('Pedra de Afiar', 'comum'), t(' x20 — pago 4g cada')]),
  msg('global', '14:13', padeiroDoroteu, [t('alguém sabe se '), qt('A Última Carga', 'principal'), t(' tem múltiplos finais? to no dilema da carta')]),
  msg('global', '14:14', erudita, [t('@PadeiroDoroteu sim, depende do que vc faz com a carta antes de entregar pro Volk')]),
  msg('global', '14:15', mestreEspada, [t('porra de quest bugada')]),
  msg('global', '14:16', loboNorte, [t('alguém comigo no pântano dos mortos? lvl 18+')]),
  msg('global', '14:17', k4t4, [t('to sim, daqui 5 min')]),
  ...
  msg('global', '14:20', noobMage, [t('quanto vale um item lendário?')]),
  msg('global', '14:21', mestreEspada, [t('@noobMage123 lendário ngm vende, lvl 35 vc vê uns no /global de tarde')]),
  msg('global', '14:22', raposaSul, [t('alguem fz '), qt('O Tribunal do Lobo', 'side'), t(' comigo? to perdida no claro central')]),
  msg('global', '14:23', botaVau, [t('to indo, espera 2 min')]),
];
...
export function makeOwnMessage(
  ...
  // _channel parâmetro existe pra evolução futura; por ora não diferencia.
  return msg('me', currentTimeHHMM(), { name: character.name, level: character.level, classKey: character.classKey }, [t(text)]);
}
...
/** Cor de texto pelo classKey — usada nos nomes de jogadores */
export const CLASS_COLOR_VAR: Record<ClassKey, string> = {
  guerreiro: 'var(--stat-forca)',
  ladino: 'var(--stat-agilidade)',
  mago: 'var(--stat-intelecto)',
};
